...
class _SmartWorkspaceState extends State<SmartWorkspace> with SingleTickerProviderStateMixin {
  ...
  late StoryEngineService _storyEngine;
  ...
  void initState() {
    ...
    _storyEngine = Provider.of<StoryEngineService>(context, listen: false);
  ...
  Future<void> _handlePatternPreview() async {
    ...
    showDialog(
      ...
      builder: (context) => PatternPopupPreview(
        blockCollection: widget.blockCollection,
      ...
      return Positioned(
        ...
        child: GestureDetector(
          ...
          child: ConnectedBlock(
            ...
            onValueChanged: (value) => widget.onValueChanged?.call(block.id, value) ?? '',
            onConnectionDragStart: (blockId, connectionId) {
              // Handle connection drag start
            },
            onConnectionDragUpdate: (position) {
              // Handle connection drag update
            },
            onConnectionDragEnd: (blockId, connectionId) {
              // Handle connection drag end
            },
            onConnectionDragCancel: () {
              // Handle connection drag cancel
            },
...
class ConnectionsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ..color = AppTheme.kenteGold.withOpacity(0.6)
      ...
      for (final connection in block.connections) {
        if (connection.connectedToId != null) {
          final connectedBlock = blocks.firstWhere(
            (b) => b.connections.any((c) => c.id == connection.connectedToId),
            orElse: () => block,
          );

          final startPoint = block.position + connection.position;
          final endPoint = connectedBlock.position + 
          ...
          canvas.drawCircle(startPoint, 4.0 * scale, pointPaint);
          canvas.drawCircle(endPoint, 4.0 * scale, pointPaint);
        }
      }
    }
  }
