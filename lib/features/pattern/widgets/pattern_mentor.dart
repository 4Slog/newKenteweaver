...
class _AIMentorWidgetState extends State<AIMentorWidget> with SingleTickerProviderStateMixin {
  ...
  Future<void> _initializeStoryEngine() async {
    try {
      ...
      debugPrint('Error initializing story engine: $e');
    }
  }
  ...
  Future<void> _analyzePattern() async {
    ...
    try {
      ...
      if (widget.blocks.isEmpty) {
        ...
        // Generate hint using story engine's context
        final currentStory = _storyEngine.currentStory;
        final currentNode = _storyEngine.currentNode;
        
        if (currentStory != null && currentNode != null) {
          hint = "Based on the current story context, try ${currentNode.content}";
        } else {
          hint = _getEmptyWorkspaceHint();
        }
      }
        ...
        builder: (context, constraints) {
          return ListTile(
            ...
            subtitle: Text(
              ...
              style: TextStyle(
                fontSize: 12,
                color: AppTheme.getDifficultyColor(context, widget.difficulty.displayName),
              ),
            ),
