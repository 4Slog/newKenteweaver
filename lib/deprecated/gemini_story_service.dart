...
class GeminiStoryService {
  ...
  final Map<String, dynamic> _storyState = {
    'currentArc': '',
    'unlockedConcepts': <String>[],
    'characterRelationships': <String, double>{},
    'previousChoices': <Map<String, dynamic>>[],
  };
...
''';
  ...
  String _buildStoryPrompt({
    ...
    final characterContext = _characterContext['kweku'];
    final difficultyContext = _getDifficultyContext(difficulty);
    final conceptsContext = _getConceptsContext(targetConcepts);
    final progressContext = _formatUserProgress(userProgress);
    final choicesContext = _formatPreviousChoices(previousChoices);

    return '''
    You are Kweku Anane, a modern storyteller and coding mentor.
    
    Character Context:
    ${jsonEncode(characterContext)}
    
    Story Parameters:
    - Template ID: $storyTemplateId
    ...
    Generate an engaging story segment that:
    1. Maintains Kweku's personality and teaching style
    2. Naturally incorporates coding concepts through Kente weaving
    3. Provides meaningful choices that affect the story
    4. Adapts difficulty based on user progress
    5. Includes appropriate challenges and hints
    ...
    Return the story in the following JSON format:
    {
      ...
      "challenges": [
        {
          "type": "pattern_creation|concept_quiz",
  ...
  String _getDifficultyContext(PatternDifficulty difficulty) {
    switch (difficulty) {
      case PatternDifficulty.basic:
        return '''
  ...
  String _getConceptsContext(List<String> concepts) {
    final conceptsMap = {
      'sequences': 'Basic pattern sequencing and order of operations',
      'loops': 'Pattern repetition and iteration',
      'variables': 'Using different colors and values',
      'conditions': 'Making choices in pattern creation',
      'functions': 'Creating reusable pattern components',
    };

    return concepts.map((concept) => 
      '- ${concept.toUpperCase()}: ${conceptsMap[concept] ?? "Custom concept"}'
    ).join('\n');
  }
  
  String _formatUserProgress(Map<String, dynamic>? progress) {
    if (progress == null) return 'New user, no previous progress';

    final buffer = StringBuffer('Current Progress:\n');
  ...
  String _formatPreviousChoices(List<Map<String, dynamic>>? choices) {
    if (choices == null || choices.isEmpty) {
      return 'No previous choices made';
    }

    final buffer = StringBuffer('Previous Choices:\n');
    for (final choice in choices) {
      buffer.writeln('- In "${choice['context']}": Chose "${choice['choice']}"');
    }
    return buffer.toString();
  }
  ...
  Future<void> _initializeCache() async {
    try {
      ...
      final cacheDir = Directory('${directory.path}/story_cache');
  ...
  Future<void> _cacheStoryContent(String key, dynamic content) async {
    try {
      final directory = await getApplicationDocumentsDirectory();
      final file = File('${directory.path}/story_cache/$key.json');
      await file.writeAsString(jsonEncode(content));
    } catch (e) {
      debugPrint('Error caching story content: $e');
    }
  }

  Future<dynamic> _getCachedContent(String key) async {
    try {
      final directory = await getApplicationDocumentsDirectory();
      final file = File('${directory.path}/story_cache/$key.json');
      ...
      debugPrint('Error reading cached content: $e');
    }
    return null;
  }
}
